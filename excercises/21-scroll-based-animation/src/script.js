...
const mesh2 = new THREE.Mesh(new THREE.ConeGeometry(1, 1, 32), material);

const mesh3 = new THREE.Mesh(
  new THREE.TorusKnotGeometry(0.7, 0.2, 16, 100),
  material
);
...
window.addEventListener("scroll", () => {
  scroll = window.scrollY;
    ...
    const section = Math.floor(scroll / sizes.height) ;

    if (section !== currentSection) {
        currentSection = section;
        gsap.to(
            sectionMeshes[section].rotation,
            {
                duration: 1.5,
                ease: "power2.inOut",
                x : '+=6',
...
const tick = () => {
  ...
  cameraGroup.position.x += (paralaxX - cameraGroup.position.x) * 5 * deltaTime; // normalize for various fps
